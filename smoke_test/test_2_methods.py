...
class TestMethods:
    ...
    async def test_cookidoo_get_active_subscription(self, cookidoo: Cookidoo) -> None:
        ...
        sub = await cookidoo.account.get_active_subscription()
    ...
    async def test_cookidoo_recipe_details(
        ...
        recipe_details = await cookidoo.recipes.get_details(recipe_id)
    ...
    async def test_cookidoo_search_recipes(self, cookidoo: Cookidoo) -> None:
        ...
        result = await cookidoo.recipes.search("Brötchen")
    ...
    async def test_cookidoo_shopping_list_recipe_and_ingredients(
        ...
        edited_ingredients = await cookidoo.shopping_list.edit_ingredient_items_ownership(
            [
        ...
        ingredients = await cookidoo.shopping_list.ingredients()
        assert isinstance(ingredients, list)
        assert len(ingredients) == 14

        recipes = await cookidoo.shopping_list.recipes()
        assert isinstance(recipes, list)
        assert len(recipes) == 2

        await cookidoo.shopping_list.remove_ingredient_items_for_recipes(["r59322", "r907016"])
        ingredients_empty = await cookidoo.shopping_list.ingredients()
    ...
    async def test_cookidoo_additional_items(self, cookidoo: Cookidoo) -> None:
        ...
        edited_additional_items = await cookidoo.shopping_list.edit_additional_items(
            [
                CookidooAdditionalItem(
                    **{
                        **additional_item.__dict__,
        ...
        additional_items = await cookidoo.shopping_list.additional_items()
    ...
    async def test_cookidoo_managed_collections(self, cookidoo: Cookidoo) -> None:
        ...
        added_managed_collection = await cookidoo.collections.managed.add("col500401")
        assert added_managed_collection.id == "col500401"

        managed_collections = await cookidoo.collections.managed.list()
        assert isinstance(managed_collections, list)
        assert len(managed_collections) == 1

        count_collections, count_pages = await cookidoo.collections.managed.count()
    ...
    async def test_cookidoo_custom_collections(self, cookidoo: Cookidoo) -> None:
        ...
        custom_collections = await cookidoo.collections.custom.list()
        assert isinstance(custom_collections, list)
        assert len(custom_collections) == 1

        count_collections, count_pages = await cookidoo.collections.custom.count()
        assert count_collections == 1
        assert count_pages == 1

        custom_collection_with_recipe = await cookidoo.collections.custom.add_recipes(
    ...
    async def test_cookidoo_calendar(self, cookidoo: Cookidoo) -> None:
        ...
        added_day_recipes = await cookidoo.calendar.add_recipes(
        ...
        day_recipes = await cookidoo.calendar.week(datetime.now().date())
        ...
        await cookidoo.calendar.remove_recipe(datetime.now().date(), "r907015")
        await cookidoo.calendar.remove_recipe(datetime.now().date(), "r59322")
